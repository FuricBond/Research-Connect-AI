...
def test_zero_n_plus_one_query_performance(db_session: Session):
    ...
    # Assert bounded query count: profile (1) + prefs (1) + interests (1) + candidates (<=5) + base models (1)
    # Total queries should be <= 15, never 30+ (which would indicate N+1)
    assert query_count <= 15, f"Too many queries executed ({query_count}); possible N+1 query issue."
